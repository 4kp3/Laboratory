...
class IconGrayConverterCircle(val iconSize: Int) {
    ...
    fun grayAndDrawCircle(icon: Bitmap): Bitmap {
        val mono = gray(icon, iconSize)
        return BitmapUtils.scaleBitmap(
            mono,
            iconSize,
            THEMED_ICON_SCALE_RATIO,
            //yixiong
//            outScale,不进行缩放，传入的时候已经缩放
            1F,
            Config.ALPHA_8
        )
    }
    ...
    private fun gray(icon: Bitmap, size: Int): Bitmap {
        ...
        val mostGray = getMostColorGray(iconDrawable)

        val bitmap =
            if (icon.width == icon.height && icon.width == size) icon else BitmapUtils.toSquareBitmap(
                iconDrawable,
                iconSize
            )
        // 获取灰度和透明度
