...
public class ClockDrawableWrapper extends AdaptiveIconDrawable implements BitmapInfo.Extender {
    ...
    private static ClockDrawableWrapper forExtras(
        ...
        // modify by henry {@
        checkForegroundLayer(info,foreground);
        // @}
        ...
        if (ATLEAST_T && aid.getMonochrome() instanceof LayerDrawable) {
            ...
            // modify by henry {@
            // 时钟应用给过来的Monochrome图标，不包含秒针图标层，这里需要过滤
            checkForegroundLayer(wrapper.mThemeInfo, (LayerDrawable) aid.getMonochrome());
            // @}
        }
    ...
    // add by henry {@
    private static void checkForegroundLayer(AnimationInfo info, LayerDrawable foreground) {
        ...
        // benny add for ABR13T-3167 @{
        sanitizeLayerIndex(info, foreground);
        // @}
    }

    /*
    * The layer index from mete data always be hourLayerIndex——0,
    * minuteLayerIndex——1, secondLayerIndex——2.
    * However, the order in foreground drawable maybe not exactly the same. It will cause ABR13T-3167.
    *
    * Hence, when we get valid layer index, we should sanitize them to match the order in foreground drawable
    * */
    private static void sanitizeLayerIndex(AnimationInfo info, LayerDrawable foreground) {
        int[] rotateDrawablePositionArray = {INVALID_VALUE, INVALID_VALUE, INVALID_VALUE};
        int rotateDrawableIndex = 0;

        // Record hour/min/second index in LayerDrawable
        int layerCount = foreground.getNumberOfLayers();
        for (int index = 0; index < layerCount; index++) {
            if (rotateDrawableIndex < rotateDrawablePositionArray.length) {
                if (foreground.getDrawable(index) instanceof RotateDrawable) {
                    rotateDrawablePositionArray[rotateDrawableIndex] = index;
                    rotateDrawableIndex++;
                }
            } else {
                break;
        ...
        // Fix wrong index provided by mete data
        if (info.hourLayerIndex != INVALID_VALUE &&
                rotateDrawablePositionArray[0] != info.hourLayerIndex) {
            Log.w(TAG, "sanitizeLayerIndex: hourLayerIndex from " + info.hourLayerIndex
                    + " to " + rotateDrawablePositionArray[0]);
            info.hourLayerIndex = rotateDrawablePositionArray[0];
        }
        if (info.minuteLayerIndex != INVALID_VALUE
                && rotateDrawablePositionArray[1] != info.minuteLayerIndex) {
            Log.w(TAG, "sanitizeLayerIndex: minuteLayerIndex from " + info.minuteLayerIndex
                    + " to " + rotateDrawablePositionArray[1]);
            info.minuteLayerIndex = rotateDrawablePositionArray[1];
        }
        if (info.secondLayerIndex != INVALID_VALUE
                    && rotateDrawablePositionArray[2] != info.secondLayerIndex) {
            Log.w(TAG, "sanitizeLayerIndex: secondLayerIndex from " + info.secondLayerIndex
                    + " to " + rotateDrawablePositionArray[2]);
            info.secondLayerIndex = rotateDrawablePositionArray[2];
        }
    }
    // @}
    ...
    public ClockBitmapInfo getExtendedInfo(Bitmap bitmap, int color,
        ...
        AnimationInfo themeInfo =
                // Benny add for NOS-607 @{
//                iconFactory.mMonoIconEnabled
                iconFactory.isMonoIconEnabled()
                // @}
                        ? mThemeInfo : null;
