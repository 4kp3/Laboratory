...
import static com.android.launcher3.icons.ShadowGenerator.NT_ADAPTIVE_ICON_SCALE_FACTOR;
...
public class IconNormalizer {
    ...
    public static float normalizeAdaptiveIcon(Drawable d, int size, @Nullable RectF outBounds) {
        ...
        // modify by henry {@
        if (ShadowGenerator.CURRENT_SCALE) {
            return 1 - BLUR_FACTOR;
        } else {
    ...
    public synchronized float getScale(@NonNull Drawable d, @Nullable RectF outBounds,
        ...
        if (d instanceof AdaptiveIconDrawable) {
            // modify by henry {@
            if (ShadowGenerator.CURRENT_SCALE) {
                // benny add for NOS-1373 @{
                mAdaptiveIconScale = NT_ADAPTIVE_ICON_SCALE_FACTOR;
                // @}
            } else {
    ...
    // add by henry {@
    public static int getNormalFolderSize(int size) {
        return Math.round(size * (1 - BLUR_FACTOR));
    }

    public static int formatBigSize(int size) {
        return Math.round(size / (1 - BLUR_FACTOR));
    }
    // @}
}
