...
import static com.android.launcher3.icons.BitmapInfo.FLAG_DUAL_APP;
...
import com.nothing.utils.DrawableExtKt;
...
public class BaseIconFactory implements AutoCloseable {
    ...
    // Add by stephen.bi for NOS-1021 @{
    private final IconGrayConverter mConverter;
    // @}
    ...
    // Instead of maintaing this value, we prefer to read the latest value from memory cache
//    protected boolean mMonoIconEnabled;
    ...
    protected BaseIconFactory(Context context, int fillResIconDpi, int iconBitmapSize,
        ...
        clear();
        // Add by stephen.bi for NOS-1021 @{
        mConverter = new IconGrayConverter(mIconBitmapSize);
        // @}
    }
    ...
    public BitmapInfo createBadgedIconBitmap(@NonNull Drawable icon,
        ...
        /*} else if (
            // Benny add for NOS-607 @{
                isMonoIconEnabled()*//*mMonoIconEnabled*//*
                        // @}
                        && IconProvider.ATLEAST_T
        ...
        } else if (isMonoIconEnabled() && IconProvider.ATLEAST_T) {
            /*
             * 若支持调整Themed Icons的大小，则走我们自己的构造THEMED ICONS的方法从而创建指定大小的THEMED ICON
             * */
            if (IconPackManager.Companion.getInstance().getSupportCustomizeThemedIconSize()) {
                if (!setNTMono(icon, originalIcon, info, scale, options)) {
                    setMonoIcon(icon, info, scale);
                }
            } else {
                setMonoIcon(icon, info, scale);
            }
    ...
    // Add by stephen.bi for NOS-1021 @{
    private void setMonoIcon(Drawable icon, BitmapInfo info, float[] scale) {
        if (icon instanceof AdaptiveIconDrawable) {
            Drawable mono = ((AdaptiveIconDrawable) icon).getMonochrome();
            if (mono != null) {
                // Convert mono drawable to bitmap
                Drawable paddedMono = new ClippedMonoDrawable(mono, -getExtraInsetFraction());
                info.setMonoIcon(
                        createIconBitmap(paddedMono, scale[0], mIconBitmapSize, Config.ALPHA_8),
                        this);
            }
    ...
    private boolean setNTMono(
        ...
        // 根据Google的图标设计指南，设置一个Launcher图标的缩放量
        mConverter.setOutScale(scale[0]);
        mConverter.setBadForeground(isBadForeground(options));

        Pair<Integer, Drawable> monochromeInfo =
                DrawableExtKt.getMonochrome(icon, mContext.getResources(), originalIcon, mConverter);
        if (monochromeInfo == null || monochromeInfo.second == null) {
            return false;
        }
        Bitmap mono;
        // 创建的
        if (monochromeInfo.second instanceof BitmapDrawable) {
            ...
            mono = createIconBitmap(monochromeInfo.second, scale[0], mIconBitmapSize, Config.ALPHA_8);
        }
        info.setMonoIcon(mono, this, monochromeInfo.first);
        return true;
    }

    private boolean isBadForeground(IconOptions options) {
        if (options instanceof IconRequest) {
            IconRequest iconRequest = (IconRequest) options;
            String packageName = iconRequest.getPackageName();
            if (packageName != null) {
                return DrawableExtKt.isBadForeground(packageName);
            }
        }
        return false;
    }
    ...
    public FlagOp getBitmapFlagOp(@Nullable IconOptions options) {
        ...
        if (options != null) {
            ...
            if (options.mUserHandle != null) {
                ...
                if (DualAppUtil.INSTANCE.isDualAppUser(options.mUserHandle)) {
                    op = op.setFlag(FLAG_DUAL_APP, isBadged);
                } else {
    ...
    public static class ClippedMonoDrawable extends InsetDrawable {
        ...
        /**
         * @param inset 如果图片内容需要放大 t 倍，则参数填入 (1 - t) / 2t
         */
    ...
    /********************************* NothingOS extends start *********************************/
    public int getFillResIconDpi() {
        return mFillResIconDpi;
    }

    // Benny add for NOS-607 @{

    /**
     * Return true if the themed icon pack is picked.
     * This api is not time consuming, it will read the value in memory
     */
    protected boolean isMonoIconEnabled() {
        return IconPackManager.Companion.getInstance().isThemedIconSelected();
    }
    // @}
    /********************************* NothingOS extends end *********************************/
}
